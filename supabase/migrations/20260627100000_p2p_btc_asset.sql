...
BEGIN
  ...
  SELECT trim(coalesce(email, '')) INTO inv_email FROM public.investors WHERE user_id = mo.investor_user_id;
  IF trim(coalesce(inv_email, '')) = '' THEN RAISE EXCEPTION 'investor email not found'; END IF;
  ...
  INSERT INTO public.withdrawals (user_id, investor_email, amount, wallet_address, payment_method, status, merchant_order_id)
  VALUES (mo.investor_user_id, inv_email, bump, 'P2P — settled to merchant', 'p2p', 'approved', mo.id);
