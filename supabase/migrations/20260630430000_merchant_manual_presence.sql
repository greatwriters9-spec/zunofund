-- Merchant visibility: auto (dashboard heartbeat) + manual stay-online / stay-offline.
...
BEGIN
  ...
  IF mode NOT IN ('auto', 'manual_online', 'manual_offline') THEN
  ...
  SET
    presence_mode = mode,
    is_online = CASE
      WHEN mode = 'manual_online' THEN true
      WHEN mode = 'manual_offline' THEN false
      ELSE mp.is_online
    END,
    last_seen_at = CASE
      WHEN mode = 'manual_online' THEN (NOW() AT TIME ZONE 'UTC')
      WHEN mode = 'auto' AND coalesce(mp.is_online, false) THEN (NOW() AT TIME ZONE 'UTC')
