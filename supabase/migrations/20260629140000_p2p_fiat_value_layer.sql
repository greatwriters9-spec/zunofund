COMMENT ON COLUMN public.merchant_offers.min_limit IS
  'Minimum trade size in offer fiat_currency_code (native fiat units).';
COMMENT ON COLUMN public.merchant_offers.max_limit IS
  'Maximum trade size in offer fiat_currency_code (native fiat units).';

CREATE OR REPLACE FUNCTION public._p2p_usd_per_unit(p_code text)
RETURNS numeric
LANGUAGE sql
STABLE
...
AS $$
  SELECT CASE
    WHEN upper(trim(coalesce(p_code, ''))) IN ('', 'USD') THEN 1::numeric
    WHEN upper(trim(p_code)) = 'USDT' THEN 1::numeric
    ELSE coalesce(
      (SELECT er.usd_value FROM public.exchange_rates er WHERE er.code = upper(trim(p_code))),
      1::numeric
    )
  END;
$$;

CREATE OR REPLACE FUNCTION public._p2p_to_usd(p_amount numeric, p_code text)
RETURNS numeric
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT round(coalesce(p_amount, 0) * public._p2p_usd_per_unit(p_code), 12);
$$;

CREATE OR REPLACE FUNCTION public._p2p_from_usd(p_usd numeric, p_code text)
RETURNS numeric
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  SELECT round(coalesce(p_usd, 0) / nullif(public._p2p_usd_per_unit(p_code), 0), 12);
$$;

CREATE OR REPLACE FUNCTION public._p2p_asset_from_side(p_side text)
RETURNS text
LANGUAGE sql
IMMUTABLE
AS $$
  SELECT CASE WHEN p_side LIKE '%_btc' THEN 'BTC' ELSE 'USDT' END;
$$;
...
BEGIN
  ...
  SELECT coalesce(sum(
    CASE
      WHEN mo.side = 'buy_btc' THEN public._p2p_to_usd(coalesce(mo.btc_escrow_amount, 0), 'BTC')
      ELSE public._p2p_to_usd(coalesce(mo.usdt_escrow_amount, 0), 'USDT')
    END
  ), 0)
  INTO pend_usd
  ...
  WHERE mo.investor_user_id = p_user_id
    ...
    AND coalesce(mo.defer_investor_deduction_until_release, false)
  ...
  RETURN greatest(
    ...
    public._p2p_to_usd(coalesce(inv.withdrawable_profit, 0), 'USDT')
      ...
      + public._p2p_to_usd(coalesce(inv.btc_withdrawable, 0), 'BTC')
      - pend_usd
  );
END;
$$;
...
SET
  min_limit = round(
    o.min_limit
      * public._p2p_usd_per_unit(public._p2p_asset_from_side(o.side))
      / nullif(public._p2p_usd_per_unit(o.fiat_currency_code), 0),
    4
  ),
  max_limit = round(
    o.max_limit
...
WHERE o.min_limit > 0 AND o.max_limit >= o.min_limit;
